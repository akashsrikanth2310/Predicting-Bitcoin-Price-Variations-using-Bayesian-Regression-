...
def findTheSimilarityValues(X,rowVal,meanofX,stdOfX,MeanofRow,stdofRow,M,counter):  #function to find the similarity values
    similarity = 0.0
    iterator=0
    counter =0
    while iterator<M:                                               #iterator runs with the M value
        subVal1 = X[iterator] - meanofX                             #subtracts with np.mean value of X
        subVal2 = rowVal[iterator] - MeanofRow                      #subtracts with np.mean value of row 
        mulVal = subVal1 * subVal2
        similarity = similarity + mulVal
        iterator = iterator + 1
    mulVal2 = stdOfX * stdofRow                      
    similarity = similarity / mulVal2
    counter +=1
    similarity = similarity / M                                     #divides by M*std of both x and row
    return similarity

def findMean(x):                      #function to find np.mean()
    return np.mean(x)

def findStd(x):                        #function to find np.std()
    return np.std(x)
 
def exponentCalculator(x):            #function to find exponent
     return math.exp(x) 
...
def computeDelta(wt, X, Xi):
    ...
    c = wt                            #assigned c value
    lengthOfX = len(X) 
    M = lengthOfX-1                   #length of M   
    meanofX = findMean(X[:-1])        #mean for X
    stdOfX = findStd(X[:-1])          #std of X
    ValueExpectedToBePredicted = 0.0
    counter = 0.0
    FractionNum = 0.0
    FractionDeno = 0.0
    for rowInd, rowVal in Xi.iterrows():    #iterates through iterrows()
        yi = rowVal[-1]                     
        counter += 1
        MeanofRow = findMean(rowVal[:-1])  
        stdofRow  = findStd(rowVal[:-1])
        SimilarityFound = findTheSimilarityValues(X,rowVal,meanofX,stdOfX,MeanofRow,stdofRow,M,counter)  #calls similarity function
        multipliedVal = c * SimilarityFound           #multiples with c
        expVal = exponentCalculator(multipliedVal)    #exponent value calculation
        FractionNum =  FractionNum + (yi * expVal)
        FractionDeno = FractionDeno + expVal
    ValueExpectedToBePredicted = float(FractionNum) / float(FractionDeno)

    return ValueExpectedToBePredicted
